...
class EnsembleModel(torch.nn.Module):
    ...
    def encode(self, src, src_key_padding_mask):
        ...
        for model in self.models:
            memory, pad = model.encode(src, src_key_padding_mask)
            memories.append(memory)
            pads.append(pad)
        return memories, pads
...
def beam_search_decode(model, src, src_key_padding_mask, bos_id, eos_id, pad_id,
    ...
    encoded = model.encode(src, src_key_padding_mask)
    if is_ensemble:
        raw_memory, raw_pads = encoded
        memory = [m.expand(beam_size, -1, -1) for m in raw_memory]
        # Ensemble members share a frame mask. Pooled models are not ensembled:
        # each one would hand the decoder a different memory length.
        src_key_padding_mask = raw_pads[0]
    else:
        raw_memory, src_key_padding_mask = encoded
