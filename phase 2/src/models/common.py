...
class PoseToTextModel(nn.Module):
    """Encoder-agnostic seq2seq wrapper: FrontEnd -> encoder -> shared Transformer decoder
    -> tied output projection.

    `encoder.forward(x, src_key_padding_mask=...)` returns (B, T, d_model), or a pair
    (memory, memory_padding_mask) when the encoder shortens the sequence it hands to
    the decoder. The phase-2 pooling arms do the latter. The mask the decoder sees is
    always the one paired with that memory, not the raw frame mask.
    """
